...
class QNetworkAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.responsible_outputs = tf.gather(tf.reshape(self.output, [-1]), self.indexes) # ??? wybrane wyjścia
        # funkcja ta bierze slice z transponowanego bufora akcji z pozycji danych w indexes

        self.loss = -tf.reduce_mean(tf.log(self.responsible_outputs)*self.reward_holder) # obliczamy średnią po wybranych wyjściach
